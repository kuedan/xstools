...
which unzip >/dev/null 2>&1 || {                                                                                                          echo -e "$print_error Couldn't find unzip, which is required." >&2
    exit 1
}
...
for folder in $package_folders; do
    for map_pk3 in $folder/*.pk3; do
        map_bsp=$(unzip -l $map_pk3 |grep -Eo '[A-Za-z0-9_-]+.bsp')
        map_name=${map_bsp%\.bsp}
        if [[ -z $map_name ]]; then
            continue
        fi  
        # use false parameters for all variables which can be set to true
        search_in_pk3=false
        search_in_autogenerated=false
        # we have the map name... and can search for a mapinfo file in different folders/files
        # first we search data/maps
        #echo -e "$print_info Map: $map_name ..."
        if [[ -f $userdir/data/maps/$map_name.mapinfo ]]; then
            #echo -e "        checking data/maps/$map_name.mapinfo"
        ...
        # search in pk3 package
        if [[ "$search_in_pk3" == "true" ]]; then
            #echo -e "        checking $folder/...pk3 file"
            unzip -p ${userdir}/${folder}/${map_pk3} maps/$map_name | grep -Eo "^(gametype|type) $@" >/dev/null &&
            maplist="$map_name $maplist"
            # we have check the used mapinfo file
            continue
        else
            search_in_autogenerated=true
        fi
        # last chance an autogenerated one 
        if [[ "$search_in_autogenerated" == "true" ]] && [[ -f $userdir/data/data/maps/autogenerated/$map_name.mapinfo ]]; then
            #echo -e "        checking .../autogenerated/$map_name.mapinfo"
